...
export default async function PaginaInvitacion({ params }: { params: Promise<{ token: string }> }) {
  ...
  const tokenValido = invitationTokenSchema.safeParse(token);
  ...
  const destino = `/invitaciones/${token}`;
    ...
    <div className="flex min-h-dvh items-center justify-center bg-surface-muted px-4">
      <Card className="w-full max-w-md">
        ...
        <CardContent className="flex flex-col gap-4">
          {!tokenValido.success ? (
            <Alert variant="destructive">{t.errores.formatoInvalido}</Alert>
          ) : email ? (
            <>
              <p className="text-sm text-ink-muted">
                {t.sesionDe} <strong className="text-ink">{email}</strong>.
              </p>
              <AcceptForm token={tokenValido.data} />
            </>
          ) : (
            <>
